...
def load_dataset(opt):
    if opt.dataset == 'smmnist':
        ...
        from data.vk import VK
        train_data = VK(
            opt,
            train=True,
            data_root=opt.train_root)
        test_data = VK(
            opt,
            train=False,
            data_root=opt.val_root)
    elif opt.dataset == 'mb':
        from data.mb import MB
        train_data = MB(
            opt,
            train=True,
            data_root=opt.train_root,
            gs_root=opt.train_gs)
        test_data = MB(
            opt,
            train=False,
            data_root=opt.val_root,
            gs_root=opt.val_gs)
    elif opt.dataset == 'mb_debug':
        from data.mb_run import MB
        train_data = MB(
            opt,
            train=True,
            data_root=opt.train_root,
            gs_root=opt.train_gs,
            audio_root=opt.train_audio_root)
        test_data = MB(
            opt,
            train=False,
            data_root=opt.val_root,
            gs_root=opt.val_gs,
            audio_root=opt.val_audio_root)
        opt.dataset = 'mb'
    elif opt.dataset == 'mb_fastflownet':
        from data.mb_fastflownet import MB
        train_data = MB(
            opt,
            train=True,
            data_root=opt.train_root,
        ...
        test_data = MB(
            ...
            data_root=opt.val_root,
            gs_root=opt.val_gs)
    elif opt.dataset == 'mb_depth':
        from data.mb_depth import MB
        train_data = MB(
            opt,
            train=True,
            data_root=opt.train_root,
        ...
        test_data = MB(
            ...
            gs_root=opt.val_gs,
            depth_root=opt.depth_root)
    elif opt.dataset == 'mb_multifile':
        from data.mb_multifile import MB
        train_data = MB(
            opt,
            train=True,
            data_root=opt.train_root,
        ...
        test_data = MB(
            ...
            data_root=opt.val_root,
            gs_root=opt.val_gs)
    elif opt.dataset == 'mb_multifile_debug':
        from data.mb_multifile_debug import MB
        train_data = MB(
            opt,
            train=True,
            data_root=opt.train_root,
            gs_root=opt.train_gs,
            audio_root=opt.train_root)
        test_data = MB(
            opt,
            train=False,
            data_root=opt.val_root,
            gs_root=opt.val_gs,
            audio_root=opt.val_root)
    elif opt.dataset == 'davis':
        from data.davis import MB
        train_data = MB(
            opt,
            train=True,
            data_root=opt.train_root,
...
def normalize_data(opt, dtype, sequence):
    if opt.dataset == 'smmnist' or opt.dataset == 'smmnist2' or opt.dataset == 'kth' or opt.dataset == 'bair' or opt.dataset == 'ucf' or opt.dataset == 'kitti' or opt.dataset == 'vk' or opt.dataset == 'mb' or opt.dataset == 'mb_fastflownet' or opt.dataset == 'mb_depth' or opt.dataset == 'mb_multifile' or opt.dataset == 'mb_multifile_debug':
